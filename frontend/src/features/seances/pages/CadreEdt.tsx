// frontend/src/features/seances/pages/CadreEdt.tsx
...
const CadreEdt: React.FC = () => {
  const location = useLocation();
  const isReadOnly = location.pathname.startsWith('/admin') || location.pathname.includes('/cadre/edt/enseignants');
  const [selectedMatiere, setSelectedMatiere] = useState<string>('');
  const [selectedEnseignantId, setSelectedEnseignantId] = useState<string>('');
  const { useFindByEnseignant, create, update, remove, isCreating, isUpdating, isRemoving } = useSeances();
  const { data: userData, isLoading: isLoadingUsers } = useUsers();
  const enseignantsTous = Array.isArray(userData) ? userData.filter((u: any) => u.role === 'ENSEIGNANT' && u.actif) : (userData?.data || []).filter((u: any) => u.role === 'ENSEIGNANT' && u.actif);
  
  // Utiliser les matières de façon statique pour s'assurer que ça s'affiche toujours
  const matieresUniques = MATIERES;
  // Filter enseignants by selected matiere if any
  const enseignants = selectedMatiere 
    ? enseignantsTous.filter((e: any) => e.matiere === selectedMatiere) 
    : enseignantsTous;
  
  const { data: classes = [] } = useClasses({ actif: true });

  const { data: seances, isLoading: isLoadingSeances } = useFindByEnseignant(selectedEnseignantId, true);
  ...
  const handleOpenModal = (seance?: Seance) => {
    ...
    if (seance) {
      ...
      setFormData({
        enseignantId: selectedEnseignantId || (enseignants[0]?.id || ''),
  ...
  const getSeancesForJour = (jour: string) => {
    return selectedEnseignantId && seances ? seances.filter((s: Seance) => s.jourSemaine === jour).sort((a: Seance, b: Seance) => a.heureDebut.localeCompare(b.heureDebut)) : [];
  };
  ...
  return (
    <div className="flex flex-col md:h-full p-4 md:p-8 space-y-6 relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Emplois du Temps (Par enseignants)</h1>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full md:w-auto">
                ...
                <Select
                  ...
                  onChange={(e) => {
                      ...
                      if (selectedEnseignantId) {
                          const profSelectionne = enseignantsTous.find((ens: any) => ens.id === selectedEnseignantId);
                          if (profSelectionne && e.target.value && profSelectionne.matiere !== e.target.value) {
                  ...
                  options={[
                      ...
                      ...matieresUniques.map((m: any) => ({ value: m, label: m }))
                  ]}
                />
              </div>
              <div className="w-full sm:w-64">
                <Select
                  ...
                  className="w-full"
                  options={[
                      { value: '', label: '-- Choisir un enseignant --' },
                      ...enseignants.map((ens: any) => ({ value: ens.id, label: `${ens.prenom} ${ens.nom} - ${ens.matiere || 'N/A'}` }))
                  ]}
                />
              </div>
            </div>
      ...
      <div className="flex-1 md:min-h-0 flex flex-col bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-auto">
        {!selectedEnseignantId ? (
          <EmptyState title="Veuillez sélectionner un enseignant pour voir son emploi du temps." icon={<CalendarIcon className="w-12 h-12" />} />
        ) : isLoadingSeances ? (
          ...
          <div className="flex-1 overflow-auto">
              ...
              <div className="block md:hidden space-y-6 p-4">
                  {(!seances || seances.length === 0) ? (
                      <EmptyState title="Aucune séance programmée pour cet enseignant." icon={<CalendarIcon className="w-12 h-12" />} />
                  ) : (
                      JOURS_SEMAINE.map(jour => {
                        ...
                        return (
                            ...
                            <div className="space-y-3">
                                {js.map((s: Seance) => (
                                <div key={s.id} onClick={isReadOnly ? undefined : () => handleOpenModal(s)} className={`p-3 bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 rounded-r-lg ${isReadOnly ? '' : 'cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40'}`}>
                      ...
                      return (
                          ...
                          {horaires.map(heure => (
                              ...
                              <motion.div
                                ...
                                onClick={isReadOnly ? undefined : () => handleOpenModal(s)}
                                className={`absolute left-1 right-1 p-1.5 rounded-lg bg-blue-100 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900/60 shadow-sm ${isReadOnly ? '' : 'hover:shadow-md hover:bg-blue-200 dark:hover:bg-blue-900/60 cursor-pointer'} transition-colors z-10 group flex flex-col gap-1 overflow-y-auto card-scrollbar`}
                              >
                                <div className="flex justify-between items-start gap-1 pb-1 border-b border-blue-200/30 dark:border-blue-900/30 shrink-0">
                                    <div className="text-xs font-bold text-blue-950 dark:text-blue-50 leading-tight break-words line-clamp-2" title={`Classe ${s.classe?.niveau}-${s.classe?.numero}`}>Classe {s.classe?.niveau}-{s.classe?.numero}</div>
                                    {!isReadOnly && (
        ...
        {isModalOpen && (
          <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingSeance ? "Modifier la séance" : "Ajouter une séance"}>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!selectedEnseignantId && ( // This is for when adding broadly without selection, though 'Ajouter' is disabled if none.
                 <Select 
                   label="Enseignant" 
                   value={formData.enseignantId} 
                   onChange={(e) => setFormData({...formData, enseignantId: e.target.value})} 
                   required
                   options={enseignants.map((ens: any) => ({ value: ens.id, label: `${ens.prenom} ${ens.nom} (${ens.matiere || 'N/A'})` }))}
                 />
              )}
              ...
              <Input label="Matière" required value={formData.matiere} onChange={e => setFormData({...formData, matiere: e.target.value})} placeholder="Ex: Mathématiques" />
              <Input 
                label="Salle" 
                required
                type="number" 
